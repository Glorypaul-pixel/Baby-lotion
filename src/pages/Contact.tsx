...
const CONTACT_STYLES = `
  ...
  @keyframes floatY { 0%,100%{transform:translateY(0) rotate(0);}33%{transform:translateY(-12px) rotate(2deg);}66%{transform:translateY(-5px) rotate(-1deg);} }
...
export const Contact: React.FC = () => {
  ...
  const contactItems = [
    ...
    },
    {
      ...
      color: "#22c55e",
      grad: "from-green-400 to-green-500",
    },
    {
      icon: Mail,
      title: "Email Us",
  ...
  return (
    ...
    >
      <div
        className="ct-blob w-80 h-80 bg-orange-300 opacity-20 top-[-60px] left-[-60px]"
        style={{ animationDuration: "9s" }}
      />
      <div
        className="ct-blob w-64 h-64 bg-pink-300 opacity-15 bottom-10 right-[-40px]"
      ...
      <section className="relative pt-28 pb-12 text-center z-10">
        ...
        <p
          className={`text-xl text-gray-600 dark:text-gray-400 ct-reveal-up ${ready ? "on" : ""}`}
      ...
      {/* Ticker */}
      <div
        className="ct-ticker-wrap py-3 mb-16"
        style={{
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Reveal type="left" delay={80} className="lg:col-span-2">
            <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-xl relative overflow-hidden">
              <div
                className="ct-blob w-40 h-40 bg-orange-200 opacity-20 -top-10 -right-10"
              ...
              <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {[
                    ...
                    <div
                      key={key}
                      style={{ animation: `fadeUp .5s ease ${i * 80}ms both` }}
                    >
                      ...
                      <input
                        ...
                        className="ct-field w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium"
                      />
                    </div>
                  ))}
                </div>
                ...
                <div style={{ animation: "fadeUp .5s ease 240ms both" }}>
                  ...
                  <textarea
                    ...
                    className="ct-field w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium resize-none"
                  />
                </div>
                <button
                  type="submit"
                  disabled={submitting}
                  className="send-btn w-full py-4 text-white rounded-full font-black text-lg flex items-center justify-center gap-2 disabled:opacity-60"
                  style={{
          ...
          <div className="space-y-5">
            {contactItems.map((item, i) => (
              <Reveal key={i} type="flip" delay={i * 140}>
                <div className="ct-card group bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg relative overflow-hidden">
                  <div
                    className="ct-blob w-24 h-24 opacity-10 -top-6 -right-6"
                    style={{
                  ...
                  />
                  <div
                    className={`ct-icon w-12 h-12 bg-gradient-to-br ${item.grad} rounded-xl flex items-center justify-center mb-4 relative z-10`}
                  >
                    <item.icon className="w-6 h-6 text-white" />
                  </div>
                  <h3
                    className="text-lg font-black text-gray-900 dark:text-white mb-1 relative z-10"
                    style={{ fontFamily: "'Nunito', sans-serif" }}
                  >
                    {item.title}
                  </h3>
                  <p className="text-gray-600 dark:text-gray-400 whitespace-pre-line text-sm relative z-10 leading-relaxed">
                    {item.content}
                  </p>
                </div>
              </Reveal>
