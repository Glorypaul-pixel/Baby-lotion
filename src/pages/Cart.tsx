...
const CART_STYLES = `
  ...
  @keyframes removeItem {
    to { opacity:0; transform:translateX(60px) scale(.85); max-height:0; margin:0; padding:0; }
  }
  @keyframes addItem {
...
export const Cart: React.FC<CartProps> = ({ onNavigate }) => {
  ...
  if (!user) return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden" style={{ background:"linear-gradient(135deg,#fff7ed,#fce7f3,#fef9c3)" }}>
      <div className="cart-blob w-72 h-72 bg-orange-300 opacity-20 top-0 left-0" style={{ animationDuration:"9s" }} />
      <div className="cart-blob w-56 h-56 bg-pink-300 opacity-15 bottom-0 right-0" style={{ animationDuration:"11s" }} />
      <div className="relative z-10 text-center" style={{ animation:"fadeUp .7s ease both" }}>
        <ShoppingBag className="empty-icon w-24 h-24 mx-auto mb-6" style={{ color:"#f97316" }} />
        <h2 className="text-4xl font-black mb-4 text-gray-900" style={{ fontFamily:"'Nunito', sans-serif" }}>
          <span className="ab-shimmer">Please Sign In</span>
        </h2>
        <p className="text-gray-500 mb-8 text-lg">Sign in to view your cart and checkout</p>
        <button onClick={() => onNavigate("auth")} className="checkout-btn px-10 py-4 text-white rounded-full font-black text-lg" style={{ background:"linear-gradient(135deg,#f97316,#ec4899)" }}>
  ...
  if (cart.length === 0) return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden" style={{ background:"linear-gradient(135deg,#fff7ed,#fce7f3,#fef9c3)" }}>
      ...
      <div className="relative z-10 text-center" style={{ animation:"fadeUp .7s ease both" }}>
        ...
        <button onClick={() => onNavigate("products")} className="checkout-btn px-10 py-4 text-white rounded-full font-black text-lg" style={{ background:"linear-gradient(135deg,#f97316,#ec4899)" }}>
          Shop Now <Sparkles className="inline w-5 h-5 ml-1" />
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen overflow-x-hidden relative" style={{ fontFamily:"'Plus Jakarta Sans',sans-serif", background:"linear-gradient(135deg,#fff7ed 0%,#fce7f3 50%,#fef9c3 100%)" }}>
      <div className="cart-blob w-80 h-80 bg-orange-300 opacity-20 top-[-60px] left-[-60px]" style={{ animationDuration:"9s" }} />
      <div className="cart-blob w-56 h-56 bg-pink-300 opacity-15 bottom-20 right-[-40px]" style={{ animationDuration:"11s", animationDelay:"3s" }} />

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-16">
        <div style={{ animation: ready ? "heroTitle .8s cubic-bezier(.22,.68,0,1.2) .05s both" : "none", opacity: ready ? undefined : 0 }} className="mb-10">
          <h1 className="text-5xl md:text-6xl font-black text-gray-900 dark:text-white" style={{ fontFamily:"'Nunito', sans-serif" }}>
            Your <span className="ab-shimmer">Cart</span>
          </h1>
          <p className="text-gray-500 mt-2 font-medium">{cart.length} item{cart.length !== 1 ? "s" : ""}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {cart.map((item, i) => (
              <Reveal key={item.id} type="left" delay={i * 80}>
                <div className="cart-item bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-md flex gap-5 relative overflow-hidden" style={{ animationDelay:`${i*80}ms` }}>
                  <div className="absolute top-0 left-0 w-1 h-full rounded-l-2xl" style={{ background:"linear-gradient(180deg,#f97316,#ec4899)" }} />
                  <div className="relative w-24 h-24 rounded-xl overflow-hidden flex-shrink-0" style={{ animation:`pulseGlow 3s ease-in-out ${i*.5}s infinite` }}>
                    <img src={item.product.image_url} alt={item.product.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-black text-lg text-gray-900 dark:text-white truncate" style={{ fontFamily:"'Nunito', sans-serif" }}>{item.product.name}</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-3 line-clamp-1">{item.product.description}</p>
                    <div className="flex items-center justify-between flex-wrap gap-3">
                      <span className="text-xl font-black ab-shimmer">${(item.product.price * item.quantity).toFixed(2)}</span>
                      <div className="flex items-center gap-1">
                        <button onClick={() => handleQty(item.id, item.quantity - 1)} className="qty-btn w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
                          <Minus size={14} />
                        </button>
                        <span className="w-8 text-center font-black text-gray-900 dark:text-white" style={{ animation:"countBounce .3s ease" }}>{item.quantity}</span>
                        <button onClick={() => handleQty(item.id, item.quantity + 1)} className="qty-btn w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
                          <Plus size={14} />
                        </button>
                        <button onClick={() => handleRemove(item.id)} className="cart-remove w-8 h-8 rounded-full bg-red-50 dark:bg-red-900/20 flex items-center justify-center ml-1">
          ...
          <Reveal type="right" delay={100}>
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-xl sticky top-24 relative overflow-hidden">
              <div className="cart-blob w-32 h-32 bg-orange-200 opacity-20 -top-8 -right-8" style={{ animationDuration:"7s" }} />
              <h2 className="text-2xl font-black mb-6 text-gray-900 dark:text-white relative z-10" style={{ fontFamily:"'Nunito', sans-serif" }}>
              ...
              <div className="border-t border-gray-100 dark:border-gray-700 pt-4 mb-6 relative z-10">
              ...
              <button
                ...
                className="checkout-btn w-full py-4 text-white rounded-full font-black text-lg flex items-center justify-center gap-2 relative z-10"
              ...
              <button
                ...
                className="w-full mt-3 py-3 text-gray-500 dark:text-gray-400 rounded-full font-bold text-sm hover:text-orange-500 transition-colors relative z-10"
              >
                Continue Shopping
              </button>
            </div>
