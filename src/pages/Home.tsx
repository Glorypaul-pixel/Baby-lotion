...
import { useProducts } from "../hooks/useProducts"; // ✅ real API, not mock
...
export const Home: React.FC<HomeProps> = ({ onNavigate }) => {
  ...
  const { products, loading } = useProducts(); // ✅ fetch from backend

  // Show only featured products (first 3 if no is_featured flag from API)
  const featuredProducts = products.slice(0, 3);

  const handleAddToCart = async (
    product: ReturnType<typeof useProducts>["products"][number],
  ) => {
    // ✅ Guard: user must be logged in before hitting the API
    if (!getAccessToken()) {
      toast.error("Please log in to add items to your cart.", {
        icon: "🔒",
        duration: 4000,
      });
      onNavigate("login"); // redirect to your login page (adjust route name if needed)
      return;
    }
    ...
    try {
      ...
      // ✅ Handle auth errors that slip past the guard (e.g. expired token)
    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-50 via-pink-50 to-white dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 transition-colors duration-300">
      ...
      <section className="bg-gradient-to-r from-orange-500 to-pink-500 dark:from-orange-600 dark:to-pink-600 py-16 md:py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <button
            onClick={() => onNavigate("products")}
            className="bg-white text-orange-600 dark:text-orange-500 px-10 py-4 rounded-full text-lg font-semibold hover:shadow-2xl transition-all transform hover:scale-105"
          >
            Order Now
          </button>
        </div>
