...
type Order = {
  ...
  created_at: string;
  total_amount: number;
  shipping_address: { fullName:string; address:string; city:string; state:string; zipCode:string; };
};
...
const OrderCard: React.FC<{ order: Order; index: number }> = ({ order, index }) => {
  ...
  return (
    <div ref={ref} className={`ord-reveal ${v ? "on" : ""}`} style={{ transitionDelay: `${index * 100}ms` }}>
      <div className="ord-card bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 shadow-lg relative overflow-hidden">
        <div className="absolute top-0 left-0 w-1.5 h-full rounded-l-2xl" style={{ background: `linear-gradient(180deg,${cfg.color},#ec4899)` }} />
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-5 gap-3 sm:gap-4">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: `${cfg.color}22` }}>
              <Icon className="w-5 h-5 sm:w-6 sm:h-6" style={{ color: cfg.color }} />
            </div>
            <div>
              <h3 className="text-base sm:text-lg font-black text-gray-900 dark:text-white" style={{ fontFamily: "Syne,sans-serif" }}>Order #{order.id.toUpperCase()}</h3>
              <p className="text-xs sm:text-sm text-gray-500">{new Date(order.created_at).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <span className="status-badge px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-black text-white" style={{ background: `linear-gradient(135deg,${cfg.color},#ec4899)`, animationDelay: `${index * 100 + 200}ms` }}>{cfg.label}</span>
            <span className="text-xl sm:text-2xl font-black ab-shimmer">${order.total_amount.toFixed(2)}</span>
          </div>
        </div>
        <div className="border-t border-gray-100 dark:border-gray-700 pt-3 sm:pt-4">
          <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1.5 sm:mb-2">Shipping Address</p>
...
export const Orders: React.FC<OrdersProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setOrders([
          { id: "abc123", status: "shipped",   created_at: new Date().toISOString(), total_amount: 49.99, shipping_address: { fullName: "John Doe",   address: "123 Baby St",  city: "New York",    state: "NY", zipCode: "10001" } },
          { id: "xyz789", status: "delivered", created_at: new Date().toISOString(), total_amount: 29.99, shipping_address: { fullName: "Jane Smith", address: "456 Care Ave", city: "Los Angeles", state: "CA", zipCode: "90001" } },
        ]);
  ...
  const ticks = ["✦ MY ORDERS", "✦ ORDER TRACKING", "✦ DELIVERY STATUS", "✦ ORDER HISTORY", "✦ PREFERABLE"];
  ...
  return (
    <div className="min-h-screen overflow-x-hidden relative" style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", background: "linear-gradient(135deg,#fff7ed 0%,#fce7f3 50%,#fef9c3 100%)" }}>
      <div className="ord-blob w-80 h-80 bg-orange-300 opacity-20 hidden sm:block" style={{ position: "absolute", top: "-60px", left: "-60px", animationDuration: "9s" }} />
      <div className="ord-blob w-60 h-60 bg-pink-300 opacity-15 hidden sm:block" style={{ position: "absolute", bottom: "60px", right: "-40px", animationDuration: "11s", animationDelay: "3s" }} />
      <section className="relative pt-24 sm:pt-28 pb-8 sm:pb-12 text-center z-10 px-4">
        <div className="inline-block px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-white text-xs sm:text-sm font-black mb-4 sm:mb-6" style={{ background: "linear-gradient(135deg,#f97316,#ec4899,#fbbf24)", animation: ready ? "badgePop .6s cubic-bezier(.22,.68,0,1.4) both" : "none", opacity: ready ? undefined : 0 }}>MY ACCOUNT</div>
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-black mb-3" style={{ fontFamily: "Syne,sans-serif", animation: ready ? "heroTitle 1s cubic-bezier(.22,.68,0,1.2) .1s both" : "none", opacity: ready ? undefined : 0 }}><span className="ab-shimmer">My</span> Orders</h1>
        <p className={`text-base sm:text-xl text-gray-600 ord-reveal-up ${ready ? "on" : ""}`} style={{ transitionDelay: "350ms" }}>Track and manage all your purchases</p>
      </section>
      <div className="tick-wrap py-2.5 sm:py-3 mb-8 sm:mb-10" style={{ background: "linear-gradient(90deg,#f97316,#ec4899)", borderTop: "2px solid #fbbf24", borderBottom: "2px solid #fbbf24" }}>
        <div className="tick-track">{[...ticks, ...ticks].map((t, i) => <span key={i} className="text-white font-black text-xs sm:text-sm tracking-widest px-5 sm:px-8 flex-shrink-0">{t}</span>)}</div>
      </div>
      <div className="relative z-10 max-w-4xl mx-auto px-3 sm:px-6 lg:px-8 pb-16 sm:pb-24">
        {loading ? (
          <div className="space-y-4">{[...Array(3)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl p-5 sm:p-6 shadow-md h-28 sm:h-32 relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent" style={{ animation: `shimmerSkel 1.5s ease ${i * .2}s infinite` }} />
              <div className="absolute bottom-5 left-0 right-0 flex justify-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-orange-300 dot-1" /><div className="w-2.5 h-2.5 rounded-full bg-pink-300 dot-2" /><div className="w-2.5 h-2.5 rounded-full bg-yellow-300 dot-3" />
              </div>
            </div>
          ))}</div>
        ) : orders.length === 0 ? (
          <div className="text-center py-16 sm:py-20" style={{ animation: "fadeUp .7s ease both" }}>
            <Package className="empty-float w-16 h-16 sm:w-24 sm:h-24 mx-auto mb-5 sm:mb-6" style={{ color: "#f97316" }} />
            <h2 className="text-3xl sm:text-4xl font-black mb-3 sm:mb-4" style={{ fontFamily: "Syne,sans-serif" }}>No Orders <span className="ab-shimmer">Yet</span></h2>
            <p className="text-gray-500 mb-6 sm:mb-8 text-base sm:text-lg">Start shopping to see your orders here</p>
            <button onClick={() => onNavigate("products")} className="ord-btn px-8 sm:px-10 py-3.5 sm:py-4 text-white rounded-full font-black text-base sm:text-lg inline-flex items-center gap-2" style={{ background: "linear-gradient(135deg,#f97316,#ec4899)" }}>
              <Sparkles className="w-4 h-4 sm:w-5 sm:h-5" />Shop Now
            </button>
          </div>
        ) : (
